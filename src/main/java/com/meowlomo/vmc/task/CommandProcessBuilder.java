...
public class CommandProcessBuilder {
	...
	private void waitStreamThreadDieAndLogIt() throws InterruptedException {
		this.seInfo.join();
		logger.info("Command Input Stream \n"+this.getInfos());				    
		this.seError.join();
		logger.info("Command Error Stream \n"+this.getErrors());
	}
    ...
    public CommandProcessBuilder(File directory, List<String> command, Long timeoutDuration) {
    ...
    */
    public int execute(){
    	...
    	try {
	    	...
	    	logger.info("Direcorty:"+directory);
	    	logger.info(command.toString());
	    	logger.info("Timeout:"+timeoutDuration);
	        ...
	        if(timeoutDuration != null){	        	
				if(!this.process.waitFor(timeoutDuration, TimeUnit.SECONDS)){
					...
					if(this.process.isAlive()){
						do{
							logger.info("CommandProcessBuilder :: try to kill process again");
							this.process.destroyForcibly();
							this.process.waitFor();
						}while(this.process.isAlive());
					}
					logger.info("Forcibly destroy signal of cmd "+command+" is triggered.");
...
//				    this.seError.join();
			...
			logger.error("CMD "+command+" is interrupted while waiting for timeout.");
			...
			logger.error("an I/O error occurs for CMD "+command);
			logger.error("",e);
    ...
    public void kill(){ 
    	if(this.process!=null && this.process.isAlive()){
	    	//try {
	    		logger.info("CommandProcessBuilder : killing the process");
	    		this.process.destroyForcibly();
				//this.seError.join();
		    	//this.seInfo.join();
		    	logger.info("CommandProcessBuilder : killed the process");
			//} catch (InterruptedException e) {
			//	logger.error("Process already interrupted.",e);
			//} 
    	}else{
    		logger.info("Process is NULL or is already terminated.");
    	}
    }
